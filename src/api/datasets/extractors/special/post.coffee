...
export default (rawData) ->
	...
	if rawData[1].data.children?.last?.kind is 'more'
		...
		post.more_replies = more.data.children.map((child) -> StringFormat.datasetId('comment', child))
	...
	for commentDataset in commentDatasets
		...
		if reply_author_short_ids_tranch.size == 500
			post.reply_author_info_tranches.push(StringFormat.datasetId('user_info_bulk', ...reply_author_short_ids_tranch))
