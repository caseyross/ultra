import { checkCredentialsTimeRemaining, renewCredentials } from './authentication.coffee'
import { checkRatelimitWait, spendRatelimit } from './ratelimit.coffee'

export default {

	GET: (endpoint, query) ->
		makeApiRequest 'GET', endpoint, { query }

	PATCH: (endpoint, content) ->
		makeApiRequest 'PATCH', endpoint, { content }

	POST: (endpoint, content) ->
		makeApiRequest 'POST', endpoint, { content }

	PUT: (endpoint, content) ->
		makeApiRequest 'PUT', endpoint, { content }

}

makeApiRequest = (method, endpoint, { query, content }) ->
	new Promise ->
