...
export default (rawData, sourceID) ->
	...
	{ main: { data: post_short_ids }, sub: datasets } = extract(postListing)
	barePostDataset = datasets.find((dataset) -> ID.var(dataset.id, 1) == post_short_ids[0])
	...
	if commentsListing
		...
		{ main: { data: direct_reply_short_ids }, sub: commentDatasets } = extract(commentsListing, sourceID)
