import RateLimitError from '/src/objects/RateLimitError'
...
api_call = (httpMethod, urlPath, requestBody) ->
...
export default
	get: (endpoint, options = {}) ->
		...
		options.raw_json = 1
		api_call 'GET', endpoint + '?' + (new URLSearchParams(options)).toString()
	post: (endpoint, content = {}) ->
		...
		api_call 'POST', endpoint, new URLSearchParams(content)